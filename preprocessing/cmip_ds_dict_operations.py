    ...
    for i,ds in tqdm(ds_ddict.items()):
        ...
        for k in pic_keys:
            if _match_twosided_attrs(ds, pic_ddict[k], attrs_a,attrs_b) == len(attrs_a): #
                if hasattr(pic_ddict[k].time,'dt')==False:
                    continue
...
def store_matched_pic_fit(ds_ddict,pic_ddict,variable_id,ndeg,out_path):
    ...
    for k,ds in tqdm(ds_ddict.items()):
        if 'matched_pic_ds' in ds.attrs:
            ...
            if fs.exists(output_fn) == False:
                ...
                pic_fit = pic_ds[variable_id].polyfit(dim='time',deg=ndeg) #fit in units m/month
                pic_fit = pic_fit.chunk({'x':ds[variable_id].chunksizes['x'],'y':ds[variable_id].chunksizes['y']})
                pic_fit.attrs['slope_units'] = 'm/month'
                
                pic_fit.to_zarr(output_fn,mode='w',zarr_version=2)
                
                pic_ds.close()
                pic_fit.close()
            else:
                print(output_fn+' already exists, continuing.')
    return ds_ddict
...
def create_regridder_dict(dict_of_ddicts, target_grid_ds,how='bilinear',add_mask=False,extrap_method=None):
    ...
    for si in tqdm(source_ids):
        for ds_ddict in dict_of_ddicts.values():
            ...
            if len(matching_keys)>0:
                ...
                if add_mask:
                    ds_vars = [k for k in ds.keys() if k!='area']
                    ds['mask'] = 0*ds[ds_vars[0]].isel(time=0,member_id=0,drop=True,missing_dims='ignore') + 1
                    ds['mask'] = ds['mask'].where(np.isfinite(ds['mask']),0)
                    if ds.lon.dims == ds['mask'].dims[::-1]:
                        ds['mask'] = ds['mask'].transpose()
                
                regridders[si] = xe.Regridder(ds,target_grid_ds,how,ignore_degenerate=True,periodic=True,unmapped_to_nan=True,extrap_method=extrap_method) #create regridder for this source_id
                continue
    return regridders
...
def regrid_datasets_in_ddict(ds_ddict,regridder_dict,regrid):
    for key,ds in tqdm(ds_ddict.items()):
        ...
        if regrid=='psmsl':
            idx = xr.DataArray(
                            data=np.arange(len(regridded_ds.lat)),
                            dims=["id"],)
            regridded_ds = regridded_ds.isel(lon=idx,lat=idx)
            regridded_ds['lat'] = ('id',regridded_ds['lat'].values)
            regridded_ds['lon'] = ('id',regridded_ds['lon'].values)
